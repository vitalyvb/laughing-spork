...
class Scanner:
    ...
    def remove_possible_simple_key(self):
        ...
        for d in kk:
            del self.possible_simple_keys[d]
        ...
        if not self.check_token([StreamEndToken]):
            ...
            if not self.check_token([DocumentStartToken]):
                raise ParserError(None, None,
                        "expected '<document start>', but found {}".format(
                        self.peek_token().id),
    ...
    def process_directives(self):
        ...
        while self.check_token([DirectiveToken]):
            ...
            if token.name == 'YAML':
                ...
                if handle in list(self.tag_handles.keys()):
                    raise ParserError(None, None,
                            "duplicate tag handle {}".format(handle),
                            token.start_mark)
                self.tag_handles[handle] = prefix
        if len(list(self.tag_handles.keys()))>0:
            value = self.yaml_version, self.tag_handles.copy()
        else:
            value = self.yaml_version, None
        for key in self.DEFAULT_TAGS.keys():
            ...
            if indentless_sequence and self.check_token([BlockEntryToken]):
                ...
                if self.check_token([ScalarToken]):
                    ...
                    if block:
                        ...
                        node = 'flow'
                    token = self.peek_token()
                    raise ParserError("while parsing a {} node".format(node), start_mark,
                            "expected the node content, but found {}".format(token.id),
                            token.start_mark)
        return event
